...
def popular():
    ...
    for u in unidades:
        ...
        if created: print(f"✅ Unidade adicionada: {u['nome']}")
    ...
    for med_nome in medicamentos:
        med, created = Medicamento.objects.get_or_create(nome=med_nome)
        if created: print(f"💊 Medicamento adicionado: {med_nome}")

    print("🏁 Tudo pronto! Itapetinga está mapeada.")
